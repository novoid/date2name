...
: C:\Python36\python.exe C:\Users\USERNAME\src\date2name\date2name.py -i %*
...
My batch file is located in =C:\Users\USERNAME\bin\date2name.bat=. Now
create a lnk file for it (e.g., via Ctrl-Shift-drag), rename the lnk
file to =date2name.lnk= and move the lnk file to
...
list of selected items to a single call of date2name.
